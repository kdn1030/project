...
public class Chat1 extends AppCompatActivity {
    ...
    private String TAG = Chat1.class.getSimpleName();
    TextView txtMessage;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnConnect.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                client.start(); // start 하면 내부 클래스에서 run 메소드 돌릴것임
            }
        });
        // 메시지 전송 버튼
        ...
        Log.e(TAG, "onCreateView in Chat1");
    }
    // 1. 내부 클래스 --> 스래드로 구현
    // 서버의 IP와 PORT번호 전달
    class SocketClient extends  Thread { // 스레드를 상속받은 내부 클래스
        ...
        private SocketClient(String ip, String port) {  // public
        ...
        public void run() { // 스레드 start 요청이 되면 run 메소드 실행
            try {
                ...
                mac = info.getMacAddress(); // 맥 어드레스가 넘어감 -> 물리적인 주소
                output.writeUTF(mac); //이 때 서버에 전달하게 되면
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        class ReceiveThread extends  Thread {
            ...
            // 생성자 소켓
           private ReceiveThread(Socket socket){ //public
                this.socket=socket;
                try{
        ...
        class SendThread extends  Thread {
            ...
            private SendThread(Socket socket){ //private
                this.socket = socket;
                try{
